...
def trans_to_cuda(variable):
    if torch.cuda.is_available():
        return variable.to(torch.device("cuda:2"))
    else:
        return variable
...
def train_test(model, train_data, test_data, epoch):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        if epoch >= 0:
            ...
            neg_global_vector = neg_global_vector.reshape(
                neg_global_vector.shape[0], -1, neg_global_vector.shape[-1]
            )
            query = global_session_1.unsqueeze(1)
            key = global_session_2.unsqueeze(1)
            samples = torch.hstack(
                [key, trans_to_cuda(torch.FloatTensor(neg_global_vector))]
            )
        ...
        total_loss += loss
        total_contrast_loss += np.tanh((epoch + 1) / 6) * loss_contrast  # ? 牛逼牛逼
        if j % int(len(slices) / 5 + 1) == 0:
            print(
                "[%d/%d] Classification Loss: %.4f, Contrast Loss: %.4f, R-Drop Loss: %.4f"
                % (
                    j,
                    len(slices),
                    (loss.cpu() - (loss_contrast * contrast_loss_ratio).cpu()).item(),
                    loss_contrast.item(),
                    loss_KL.item(),
                )
            )
    ...
    print(
        "\tTotal Classification Loss:\t%.3f, Total Contrast Loss:\t%.3f, Contrast Loss Weight:\t%.3f"
        % (total_loss, total_contrast_loss, contrast_loss_ratio)
    )
    logger.info(
        "\tTotal Classification Loss:\t%.3f, Total Contrast Loss:\t%.3f, Contrast Loss Weight:\t%.3f"
        % (total_loss, total_contrast_loss, contrast_loss_ratio)
